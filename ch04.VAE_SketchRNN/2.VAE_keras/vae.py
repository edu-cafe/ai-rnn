...
def gaussian_MLP_encoder(x, n_hidden, n_output, keep_prob):
    with tf.variable_scope("gaussian_MLP_encoder"):
        # initializers
        w_init = tf.contrib.layers.variance_scaling_initializer()  # he initializer
...
def autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob):
    ...
    z = mu + sigma * tf.random_normal(tf.shape(mu), 0, 1, dtype=tf.float32)  # 0, 1:normal distribution에서 샘플링
