...
"""
## CBOW 모델의 학습
* CBOW 모델의 출력층에서 각 단어의 점수를 계산한 뒤 이 점수에 소프트맥스 함수를 적용하면 '확률'을 얻을 수 있음
* 이 확률은 맥락(전후 단어)이 주어졌을 때 그 중앙에 어떤 단어가 출현하는 지를 나타냄 
* CBOW 모델의 학습에서는 올바른 예측을 할 수 있도록 가중치를 조정하는 일을 함 
* 그 결과로 가중치 W_in과 W_out에 단어의 출현 패턴을 파악한 벡터가 학습됨

## 최종적으로 이용하는 단어의 분산표현으로는 어느 쪽 가중치를 선택하면 좋을까?
* A : 입력 측의 가중치만 이용한다
* B : 출력 측의 가중치만 이용한다
* C : 양쪽 가중치를 모두 이용한다
* word2vec(특히, skip-gram 모델)에서는 A안인 '입력 측의 가중치만 이용한다'가 가장 대중적인 선택임
* 많은 연구에서 출력 측 가중치는 버리고 입력 측 가중치 W_in만을 최종 단어의 분산 표현으로 이용함
* GloVe에서는 두 가중치를 더했을 때 좋은 결과를 얻었음 
"""
