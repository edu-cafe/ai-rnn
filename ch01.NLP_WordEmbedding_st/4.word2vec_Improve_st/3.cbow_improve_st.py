...
class CBOW:
    ...
    def __init__(self, vocab_size, hidden_size, window_size, corpus):
        ...
        self.ns_loss = _______(W_out, corpus, power=0.75, sample_size=5)
    ...
    def forward(self, context, target):
        ...
        for i, layer in enumerate(self.in_layers):
            h += layer.forward(______[:, i])
        h *= 1 / len(self.in_layers)
        loss = self.ns_loss.forward(___, ______)
        return loss
...
#%%
# Create Model
model = ____(vocab_size, hidden_size, window_size, corpus)
optimizer = Adam()
trainer = Trainer(____, ______) 

#%%
# Start Training
trainer.___(contexts, target, max_epoch, batch_size)
