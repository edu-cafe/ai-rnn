...
class NegativeSamplingLoss:
    ...
    def forward(self, h, target):
        batch_size = target.shape[0]
        negative_sample = self.sampler.________
        ...
        negative_label = np._____(batch_size, dtype=np.int32)
        for i in range(self.sample_size):
            negative_target = negative_sample[:, i]
            score = self.embed_dot_layers[i+1]._________
            loss += self.loss_layers[i+1]._________
